...
#include "Texture.h"
#include "Mouse.h"
CSelectMenu7::CSelectMenu7(Engine::MYGDI* pMyGDI)
...
CSelectMenu7::~CSelectMenu7(void)
{

}

HRESULT CSelectMenu7::Initialize(void)
{
	...
	m_pInfo->m_vPos = XMFLOAT3(0.f, -0.4f, 0.f);
...
void CSelectMenu7::Release(void)
{

}

int CSelectMenu7::Update(void)
{
	if (CMouse::GetMousePos().x >= 310 && CMouse::GetMousePos().x <= 480 &&
		CMouse::GetMousePos().y >= 400 && CMouse::GetMousePos().y <= 430)
		m_bMouseOn = true;
	else
		m_bMouseOn = false;
...
HRESULT CSelectMenu7::AddComponent(void)
{
...
CSelectMenu7* CSelectMenu7::Create(Engine::MYGDI* pMyGDI)
{
	CSelectMenu7*		pGameObject = new CSelectMenu7(pMyGDI);
