...
HRESULT CPlayer::Initialize(void)
{
	...
	//m_pInfo->m_vDir = XMFLOAT3(0, 0, 1.f);
...
int CPlayer::Update(void)
{
	...
	if (Engine::Get_Input()->OnceKeyDown(DIK_F1) )
	{
		BUTON7 = true;
	}
	else
	...
	if (Engine::Get_Input()->OnceKeyDown(DIK_F2))
	{
		BUTON8 = true;
	}
	else
...
void CPlayer::KeyCheck(void)
{
	...
	if (Engine::Get_Input()->Get_DIKeyState(DIK_W) )
	{
	...
	if (Engine::Get_Input()->Get_DIKeyState(DIK_S))
	{
		m_dwState = STATE_WALK;
	...
	if (Engine::Get_Input()->Get_DIKeyState(DIK_D) )
	{
		m_dwState = STATE_WALK;
	...
	if (Engine::Get_Input()->Get_DIKeyState(DIK_A) )
	{
		m_dwState = STATE_WALK;
		//m_pInfo->m_fAngle[Engine::ANGLE_Y] += XMConvertToRadians(90) * fTime;
		XMVECTOR VP = XMLoadFloat3(&m_pInfo->m_vPos);
	...
	if (Engine::Get_Input()->Get_DIKeyState(DIK_SPACE))
	{
		if (m_bPush == true)
			return;
		m_bPush = true;
