...
CLogoLetter::CLogoLetter(Engine::MYGDI* pMyGDI)
...
CLogoLetter::~CLogoLetter(void)
{

}

HRESULT CLogoLetter::Initialize(void)
{
	FAILED_CHECK(AddComponent());

	//m_pInfo->m_fAngle[Engine::ANGLE_Z] += 270.f;
	//m_pInfo->m_matWorld 
	m_pInfo->m_vScale = XMFLOAT3(0.3f, 0.4f, 0.4f);
	m_pInfo->m_vPos = XMFLOAT3(0.0f, -0.5f, 0.f);
...
void CLogoLetter::Release(void)
{

}

int CLogoLetter::Update(void)
{
	...
	if (m_bRight)
	{
		m_DirLight.Ambient.x -= m_fTime*0.0001f;
		m_DirLight.Ambient.y -= m_fTime*0.0001f;
		m_DirLight.Ambient.z -= m_fTime*0.0001f;
		m_DirLight.Ambient.w -= m_fTime*0.0001f;
...
HRESULT CLogoLetter::AddComponent(void)
{
...
CLogoLetter* CLogoLetter::Create(Engine::MYGDI* pMyGDI)
{
	CLogoLetter*		pGameObject = new CLogoLetter(pMyGDI);
