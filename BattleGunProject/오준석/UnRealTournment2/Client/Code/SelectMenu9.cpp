...
#include "Texture.h"
#include "Mouse.h"
CSelectMenu9::CSelectMenu9(Engine::MYGDI* pMyGDI)
...
CSelectMenu9::~CSelectMenu9(void)
{

}

HRESULT CSelectMenu9::Initialize(void)
{
	...
	m_pInfo->m_vPos = XMFLOAT3(0.f, -0.85f, 0.f);
...
void CSelectMenu9::Release(void)
{

}

int CSelectMenu9::Update(void)
{
	if (CMouse::GetMousePos().x >= 310 && CMouse::GetMousePos().x <= 480 &&
		CMouse::GetMousePos().y >= 520 && CMouse::GetMousePos().y <= 550)
		m_bMouseOn = true;
	else
		m_bMouseOn = false;
...
void CSelectMenu9::Render(void)
{
	...
	//if (m_fTime > 5.f)
	{
		m_pTexture->Render(0, 9);
		m_pBuffer->Render(m_pInfo->m_matWorld);

	}
}

HRESULT CSelectMenu9::AddComponent(void)
{
...
CSelectMenu9* CSelectMenu9::Create(Engine::MYGDI* pMyGDI)
{
	CSelectMenu9*		pGameObject = new CSelectMenu9(pMyGDI);
