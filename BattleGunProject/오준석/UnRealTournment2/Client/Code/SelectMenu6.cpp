...
#include "Texture.h"
#include "Mouse.h"
CSelectMenu6::CSelectMenu6(Engine::MYGDI* pMyGDI)
...
CSelectMenu6::~CSelectMenu6(void)
{

}

HRESULT CSelectMenu6::Initialize(void)
{
	...
	m_pInfo->m_vPos = XMFLOAT3(0.f, -0.25f, 0.f);
...
void CSelectMenu6::Release(void)
{

}

int CSelectMenu6::Update(void)
{
	if (CMouse::GetMousePos().x >= 310 && CMouse::GetMousePos().x <= 480 &&
		CMouse::GetMousePos().y >= 360 && CMouse::GetMousePos().y <= 390)
		m_bMouseOn = true;
	else
		m_bMouseOn = false;
...
void CSelectMenu6::Render(void)
{
	...
	//if (m_fTime > 5.f)
	{
		m_pTexture->Render(0, 6);
		m_pBuffer->Render(m_pInfo->m_matWorld);

	}
}

HRESULT CSelectMenu6::AddComponent(void)
{
...
CSelectMenu6* CSelectMenu6::Create(Engine::MYGDI* pMyGDI)
{
	CSelectMenu6*		pGameObject = new CSelectMenu6(pMyGDI);
