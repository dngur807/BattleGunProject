...
BOOL CMainFrame::PreCreateWindow(CREATESTRUCT& cs)
{
	...
	//cs.x = 100;
	//cs.y = 100;
	//cs.cx = tWINCX + 500;  // 자신이 원하는 메인 프레임의 x축 크기
	//cs.cy = tWINCY;  // 자신이 원하는 메인 프레임의 y축 크기
	cs.style &= ~(FWS_ADDTOTITLE | WS_THICKFRAME | WS_MAXIMIZEBOX); //| WS_MINIMIZEBOX);
	LPCTSTR titleName = L"배트맨 MapTool";
	SetTitle(titleName);
...
BOOL CMainFrame::OnCreateClient(LPCREATESTRUCT lpcs, CCreateContext* pContext)
{
	...
	m_MainSplitter.CreateView(0, 1
		, RUNTIME_CLASS(CMapEdit)
		, CSize(300, 950)
		, pContext);

	m_MainSplitter.SetColumnInfo(1, 500, 50);
	m_MainSplitter.SetColumnInfo(0, 1200, 950);

	m_pMainView = (CIncludeView*)m_MainSplitter.GetPane(0, 0);
	m_pMapEdit = (CMapEdit*)m_MainSplitter.GetPane(0, 1);
