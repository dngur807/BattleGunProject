...
int CStone::Update(void)
{
	...
	//m_pInfo->m_vPos.x++;
	m_pInfo->m_vPos.x = m_pDynamicCamera->GetEye().x;
	m_pInfo->m_vPos.z = m_pDynamicCamera->GetEye().z;
	//m_pInfo->m_vPos.y = 
	/*if(!m_pMyCell)
...
void CStone::GetStoneY(float pFloat)
{
	m_pInfo->m_vPos.y = pFloat;
}
...
HRESULT CStone::Initialize(void)
{
	...
	m_pInfo->m_vScale = XMFLOAT3(0.3f, 0.3f, 0.3f);
...
HRESULT CStone::AddComponent(void)
{
	...
	m_pMesh = Engine::CStaticMesh::Create(m_pMyGDI,
		L"../../Client/bin/Resources/Mesh/StaticMesh/TombStone/", L"TombStone.X");
	return S_OK;
}

CStone * CStone::Create(Engine::MYGDI * pMyGDI, CDynamicCamera*	pDynamicCamera)
{
	CStone* pGameObject = new CStone(pMyGDI);
	if (FAILED(pGameObject->Initialize()))
		Engine::Safe_Delete(pGameObject);

	pGameObject->m_pDynamicCamera = pDynamicCamera;
