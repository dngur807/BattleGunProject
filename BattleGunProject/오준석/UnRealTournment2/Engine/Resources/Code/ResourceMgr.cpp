	...
	, const WORD& wItv /*= 1*/)
{
	...
	case CVIBuffer::BUFFER_TILEMAP:
		pBuffer = CMapTile::Create(pMyGDI, static_cast<float>(wItv * (wCntX - 1)),
			static_cast<float>(wItv * (wCntZ - 1)), wCntX, wCntZ);
		break;
	case CVIBuffer::BUFFER_BOX:
		pBuffer = CBoxTex::Create(pMyGDI);
		break;
	case CVIBuffer::BUFFER_WAVES:
		pBuffer = CWavesTex::Create(pMyGDI);
		break;
	case CVIBuffer::BUFFER_SKYBOX:
		pBuffer = CSkyBoxTex::Create(pMyGDI);
		break;
	}
