...
public:
	...
	void RequestLoadingEnd();//로딩 끝
	void RequestPosDir(UINT iKey , XMFLOAT3 vPos, float fAngleX, float fAngleY);


	void RequestGiveDamage(int iTargetIndex, int iDamage);
	void RequestDead(int iVictim, int iKiller);
	void RequestHpSync(int index, int iHp);
	//통신 핸들러들의 모임
	...
	friend int OnNotifyLoadingEnd(char* cpPacket);


	friend int OnNotifyInput(char* cpPacket);
	friend int OnNotifyPosDir(char* cpPacket);
	friend int OnNotifyGiveDamage(char* cpPacket);
	friend int OnNotifyHpSync(char* cpPacket);
	friend int OnNotifyDead(char* cpPacket);
