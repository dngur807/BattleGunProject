...
void CCameraMgr::Initialize()
{
	...
	m_pStaticCamera = CStaticCamera::Create(Engine::Get_Management()->GetGDI());
...
void CCameraMgr::SetTargetInfo(const Engine::CTransform* pTargetInfo)
{
	...
	m_pStaticCamera->SetTargetInfo(pTargetInfo);
}
...
void CCameraMgr::SetCamera(CAMERAMODE eMode)
{
	...
	case CAMERA_FPS:
		...
		break;
	case CAMERA_STATIC:
		m_pSelectedCamera = m_pStaticCamera;
...
void CCameraMgr::StaticTrace(bool bOn)
{
	m_pStaticCamera->SetTrace(bOn);
}
