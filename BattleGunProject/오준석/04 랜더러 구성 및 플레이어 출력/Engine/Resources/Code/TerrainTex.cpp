...
Engine::CTerrainTex::CTerrainTex(MYGDI* pMyGDI)
	:CVIBuffer(pMyGDI)
{
...
HRESULT Engine::CTerrainTex::CreateBuffer(const float & fWidth, 
	const float & fDepth, const UINT & iCntX, const UINT & iCntZ)
{
	...
	for (size_t i = 0; i < tMeshInfo.Vertices.size(); ++i)
	{
		...
		vPos.y = (pdwPixel[i] & 0x000000FF) / 100.f;
...
float Engine::CTerrainTex::GetHillHeight(float x, float z) const
{
	return 0.3f*(z*sinf(0.1f*x) + x*cosf(0.1f*z));

}

DirectX::XMFLOAT3 Engine::CTerrainTex::GetHillNormal(float x, float z) const
{
	// n = (-df/dx, 1, -df/dz)
	DirectX::XMFLOAT3 n(
...
Engine::CResources * Engine::CTerrainTex::CloneResource(void)
{
	...
	CResources* pResources = new CTerrainTex(*this);

	return pResources;
}

Engine::CTerrainTex * Engine::CTerrainTex::Create(MYGDI* pMyGDI,
	const float & fWidth, const float & fDepth, 
	const UINT & iCntX, const UINT & iCntZ)
{
	CTerrainTex* pBuffer = new CTerrainTex(pMyGDI);
