...
void CCube::Render(void)
{
	...
	m_pTest->Render(m_pInfo->m_matWorld);
}
...
HRESULT CCube::AddComponent(void)
{
	...
	pComponent = m_pResourceMgr->CloneResource(RESOURCE_STAGE, L"Buffer_Sphere");
	m_pTest = dynamic_cast<Engine::CSphereTex*>(pComponent);
	NULL_CHECK_RETURN(pComponent, E_FAIL);
	m_mapComponent.insert(MAPCOMPONENT::value_type(L"CubeTex", m_pTest));
