...
void Engine::CGraphicDev::OnResize(HWND hWnd, const WORD& wSizeX, const WORD& wSizeY)
{
	...
	rsDesc.CullMode = D3D11_CULL_NONE;//선별을 비활성화 후면 삼각형 선별해서 제외하려면 true
	rsDesc.FrontCounterClockwise = false;//시계방향이 전면
	m_pDevice->CreateRasterizerState(&rsDesc, &m_pWire);


	rsDesc.FillMode = D3D11_FILL_SOLID;
	m_pDevice->CreateRasterizerState(&rsDesc, &m_pSolid);
...
void Engine::CGraphicDev::WireFrame()
{
	m_pImmediateContext->RSSetState(m_pWire);
}

void Engine::CGraphicDev::SolidFrame()
{
