...
#include "Texture.h"
#include "Mouse.h"
CSelectMenu2::CSelectMenu2(Engine::MYGDI* pMyGDI)
...
CSelectMenu2::~CSelectMenu2(void)
{

}

HRESULT CSelectMenu2::Initialize(void)
{
	...
	m_pInfo->m_vPos = XMFLOAT3(0.f, -0.7f, 0.f);
...
void CSelectMenu2::Release(void)
{

}

int CSelectMenu2::Update(void)
{
	if (CMouse::GetMousePos().x >= 310 && CMouse::GetMousePos().x <= 480 &&
		CMouse::GetMousePos().y >= 480 && CMouse::GetMousePos().y <= 510)
		m_bMouseOn = true;
	else
		m_bMouseOn = false;
...
HRESULT CSelectMenu2::AddComponent(void)
{
...
CSelectMenu2* CSelectMenu2::Create(Engine::MYGDI* pMyGDI)
{
	CSelectMenu2*		pGameObject = new CSelectMenu2(pMyGDI);
