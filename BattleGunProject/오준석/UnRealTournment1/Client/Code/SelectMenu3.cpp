...
CSelectMenu3::CSelectMenu3(Engine::MYGDI* pMyGDI)
...
CSelectMenu3::~CSelectMenu3(void)
{

}

HRESULT CSelectMenu3::Initialize(void)
{
	...
	m_pInfo->m_vPos = XMFLOAT3(0.f, 0.2f, 0.f);
...
void CSelectMenu3::Release(void)
{

}

int CSelectMenu3::Update(void)
{
	if (CMouse::GetMousePos().x >= 310 && CMouse::GetMousePos().x <= 480 &&
		CMouse::GetMousePos().y >= 230 && CMouse::GetMousePos().y <= 260)
		m_bMouseOn = true;
	else
		m_bMouseOn = false;
...
HRESULT CSelectMenu3::AddComponent(void)
{
...
CSelectMenu3* CSelectMenu3::Create(Engine::MYGDI* pMyGDI)
{
	CSelectMenu3*		pGameObject = new CSelectMenu3(pMyGDI);
