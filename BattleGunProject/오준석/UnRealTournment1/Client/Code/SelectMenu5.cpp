...
#include "Texture.h"
#include "Mouse.h"
CSelectMenu5::CSelectMenu5(Engine::MYGDI* pMyGDI)
...
CSelectMenu5::~CSelectMenu5(void)
{

}

HRESULT CSelectMenu5::Initialize(void)
{
	...
	m_pInfo->m_vPos = XMFLOAT3(0.f, -0.1f, 0.f);
...
void CSelectMenu5::Release(void)
{

}

int CSelectMenu5::Update(void)
{
	if (CMouse::GetMousePos().x >= 310 && CMouse::GetMousePos().x <= 480 &&
		CMouse::GetMousePos().y >= 320 && CMouse::GetMousePos().y <= 350)
		m_bMouseOn = true;
	else
		m_bMouseOn = false;
...
HRESULT CSelectMenu5::AddComponent(void)
{
...
CSelectMenu5* CSelectMenu5::Create(Engine::MYGDI* pMyGDI)
{
	CSelectMenu5*		pGameObject = new CSelectMenu5(pMyGDI);
