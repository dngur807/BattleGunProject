...
void CReactUI_2D::SetScale(float fX, float fY)
{
	m_pInfo->m_vScale = XMFLOAT3(fX, fY, 1.f);

	//z크기 안맞네;; SetCale size 이거아님
	m_fSizeX = fY;
	m_fSizeY = fX;
}

void CReactUI_2D::UIRoationZ(float fAngle , float fScaleX , float fScaleY)
{
	m_pInfo->m_vScale.x = fScaleX;
	m_pInfo->m_vScale.y = fScaleY;
	m_pInfo->m_fAngle[Engine::ANGLE_Z] = XMConvertToRadians(270 - fAngle);
